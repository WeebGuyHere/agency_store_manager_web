...
//policy for store. By Default: Max-type-store: 7; Max-store/type: 8
$qry = mysqli_query($conn,"SELECT * FROM tochucdaily"); 
$data = mysqli_fetch_array($qry); 
if (isset ($_SESSION['username']) && $_SESSION['level'] == 1 && ($_SESSION['security_code'])) 
{
    if(isset($_POST['Update'])) 
    {
        $maxtype = $_POST['maxtype'];
        $maxstore = $_POST['maxstore']; 
            $sql="UPDATE tochucdaily SET SoLoaiDaiLy ='$maxtype', SoDaiLyToiDa ='$maxstore'";
            ...
            if ($result) 
            {
                ?>
                <script>
                    if (!alert('Edit policy success!')) {
                        document.location = "./AgentOrganization.php";
                    }
                </script>
                <?php
                mysqli_close($conn);
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <div id="content">
                ...
                <div class="container-fluid" style="margin: auto; width: 50%;">
                    ...
                    <!-- Page Heading -->
                    <h1 class="h3 mb-2 text-gray-800">Chính sách số các đại lý và loại tối đa</h1>
                    <br>

                    <!-- DataTales Store -->
                    <div class="card shadow mb-4" style="width: 50rem;">
                        <div class="card-header py-3">
                          <h4>Thay đổi tối đa</h4>  
                        </div>
                        <div class="card-body">
                        <form method="POST">
                        <div class="form-group row">
                        <label class="col-sm-2 col-form-label">Số loại đại lý tối đa</label>
                        <div class="col-sm-10">
                        <input type="text" name="maxtype" value="<?php echo $data['SoLoaiDaiLy'] ?>" class="form-control" placeholder="Nhập tối đa" required>
                        </div>
                      </div>
                      <div class="form-group row">
                        <label class="col-sm-2 col-form-label">Số đại lý/loại tối đa</label>
                        <div class="col-sm-10">
                        <input type="text" name="maxstore" value="<?php echo $data['SoDaiLyToiDa'] ?>" class="form-control" placeholder="Nhập tối đa" required>
                        </div>
                      </div>
                        <div class="form-group row">
                        <div class="col-sm-10">
