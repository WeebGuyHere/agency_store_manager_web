...
$id = $_GET['ID']; 
$qry = mysqli_query($conn,"SELECT * FROM phieuthutien where MaPhieuThu ='$id'"); 
    ...
    if(isset($_POST['Update'])) 
    {
        ...
        $data = mysqli_fetch_object($query0); 
            if ($price < $data->TienNo || $price == $data->TienNo || $data->TienNo == 0)
            {
                ...
                $addr = $data->DiaChi;
                $email = $data->Email;
                $query = "UPDATE phieuthutien SET MaDaiLy ='$idstore',
                    DienThoai ='$tel', DiaChi ='$addr',
                    Email ='$email', NgayThuTien =' $date',
                    SoTienThu ='$price' WHERE MaPhieuThu = '$idbill'";
                $run = mysqli_query($conn,$query);
                if ($run)
                {
                ?>
                        <script>
                            if (!alert('Edit Bill success!')) {
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <div id="content">
                ...
                <div class="container-fluid" style="margin: auto; width: 50%;">
                    ...
                    <!-- Page Heading -->
                    <h1 class="h3 mb-2 text-gray-800">Sửa thông tin thiếu thu</h1>
                    <br>

                    
                    <div class="card shadow mb-4" style="width: 50rem;">
                        <div class="card-header py-3">
                          <h4>Sửa thông tin phiếu thu</h4>  
                        </div>
                        <div class="card-body">
                        <form method="POST">
                        <div class="form-group">
                         <input type="text" name="idbill" value = "<?php echo $data0['MaPhieuThu'] ?>" class="form-control" placeholder="Nhập mã phiếu thu" readonly>
                            </div>
                            <div class="form-group">
                                <select class="form-control" name="idstore" required>
                                <option value="<?php echo $data0['MaDaiLy'] ?>"><?php echo $data0['MaDaiLy'] ?></option>
                                <option value="">Chọn mã đại lý</option>
                                <?php
                                //get list id store
                                    $list_store = "SELECT * FROM cacdaily";
                                    $list = mysqli_query($conn,$list_store); 
                                    while($data2=mysqli_fetch_array($list))
                                    {
                                ?>
                                    <option value="<?php echo $data2['MaDaiLy']; ?>"><?php echo $data2['MaDaiLy']; ?></option>
                                <?php
                                    }
                                ?>
                                </select>
                            ...
                            <div class="form-group">
                                <div class="dates">
                                    <input type="text" name="date" value="<?php echo $data0['NgayThuTien'] ?>" id="date" class="form-control" placeholder="YYYY-MM-DD" autocomplete="off"  required>
                                </div>               
                            </div>
                            <div class="form-group">
                                <input type="text" name="price" value="<?php echo $data0['SoTienThu'] ?>" class="form-control" placeholder="Nhập tiền thu" required>
                            </div>
                            <div class="modal-footer">
                                <a href="../Bill.php" class="btn btn-secondary active" role="button" aria-pressed="true">Huỷ</a>
